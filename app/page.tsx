...
function FitnessApp({user}:{user:User}) {
  ...
  async function save(entry:CheckIn,photos:Partial<Record<PhotoAngle,File>>={}){try{await cloudFitnessRepository.save(entry,user.id);for(const [angle,file] of Object.entries(photos) as Array<[PhotoAngle,File]>)await cloudFitnessRepository.uploadPhoto(entry,user.id,angle,file);const photoCount=Object.keys(photos).length;await refresh(photoCount?`Check-in and ${photoCount} ${photoCount===1?"photo":"photos"} securely synced`:editing==="new"?"Check-in securely synced":"Check-in updated")}catch(error){setNotice(error instanceof Error?error.message:"Unable to save")}}
...
function PhotoGallery({entries,onAdd,onUpload}:{entries:CheckIn[];onAdd:()=>void;onUpload:(angle:PhotoAngle,file?:File)=>Promise<void>}) {
  ...
  return <><section className="photo-intro"><div className="photo-lock">⌾</div><p className="eyebrow">PRIVATE BY DESIGN</p><h2>{count ? `${count} progress photos` : "Your photo timeline starts here."}</h2><p>Photos are stored privately in Supabase. Only the signed-in owner can retrieve them.</p><button className="primary" onClick={onAdd}>Start photo check-in</button></section><section className="photo-grid" aria-label="Upload latest progress photo angles">{(["front","side","back"] as PhotoAngle[]).map(angle=>{const exists=latest.photos.some(photo=>photo.angle===angle);return <label key={angle}><input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>onUpload(angle,event.target.files?.[0])}/><span>＋</span><strong>{angle[0].toUpperCase()+angle.slice(1)}</strong><small>{exists?"Replace latest photo":"Upload to latest check-in"}</small></label>})}</section><div className="soft-note"><strong>Consistency tip</strong><p>Use the same room, distance, lighting and relaxed pose each time for useful comparisons.</p></div></>;
}

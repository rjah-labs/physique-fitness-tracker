...
function FitnessApp({user}:{user:User}) {
  ...
  async function uploadLatest(angle:PhotoAngle,file?:File){if(!file)return;try{setNotice("Uploading securely…");await cloudFitnessRepository.uploadPhoto(latest,user.id,angle,file);await refresh(`${angle[0].toUpperCase()+angle.slice(1)} photo uploaded`)}catch(error){notify(error instanceof Error?error.message:"Upload failed")}}
  ...
  return <main className="app-shell">
    ...
    {tab === "Measure" && <MeasurementsArea entries={checkIns} onAdd={() => setEditing("new")} onEdit={setEditing} onPhotoUpload={uploadLatest}/>}
...
function MeasurementsArea({entries,onAdd,onEdit,onPhotoUpload}:{entries:CheckIn[];onAdd:()=>void;onEdit:(entry:CheckIn)=>void;onPhotoUpload:(angle:PhotoAngle,file?:File)=>Promise<void>}) {
...
function PhotoGallery({entries,onAdd,onUpload}:{entries:CheckIn[];onAdd:()=>void;onUpload:(angle:PhotoAngle,file?:File)=>Promise<void>}) {
  const count=entries.reduce((total,item)=>total+item.photos.length,0);
  const latest=entries.at(-1)!;
  const [photoUrls,setPhotoUrls]=useState<Record<string,string>>({});const [activePhoto,setActivePhoto]=useState<{url:string;angle:PhotoAngle}|null>(null);const photoKey=latest.photos.map(photo=>photo.storagePath).join("|");
  useEffect(()=>{let cancelled=false;Promise.all(latest.photos.map(async photo=>[photo.storagePath,await cloudFitnessRepository.photoUrl(photo.storagePath)] as const)).then(pairs=>{if(!cancelled)setPhotoUrls(Object.fromEntries(pairs))}).catch(()=>undefined);return()=>{cancelled=true}},[photoKey]);
  return <><section className="photo-intro"><div className="photo-lock">⌾</div><p className="eyebrow">PRIVATE BY DESIGN</p><h2>{count ? `${count} progress photos` : "Your photo timeline starts here."}</h2><p>Photos are stored privately in Supabase. Only the signed-in owner can retrieve them.</p><button className="primary" onClick={onAdd}>Start photo check-in</button></section><p className="photo-date">LATEST · {formatDate(latest.date)}</p><section className="photo-grid" aria-label="Latest progress photos">{(["front","side","back"] as PhotoAngle[]).map(angle=>{const photo=latest.photos.find(item=>item.angle===angle);const url=photo&&photoUrls[photo.storagePath];const label=angle[0].toUpperCase()+angle.slice(1);return <article className={url?"has-photo":""} key={angle}>{url?<><button className="photo-view" onClick={()=>setActivePhoto({url,angle})} aria-label={`View ${label} photo larger`}><img src={url} alt={`${label} progress photo`}/><span>{label}</span></button><label className="photo-replace"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>onUpload(angle,event.target.files?.[0])}/>Replace</label></>:<label className="photo-empty"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>onUpload(angle,event.target.files?.[0])}/><span>＋</span><strong>{label}</strong><small>Upload to latest check-in</small></label>}</article>})}</section><div className="soft-note"><strong>Consistency tip</strong><p>Use the same room, distance, lighting and relaxed pose each time for useful comparisons.</p></div>{activePhoto&&<div className="photo-lightbox" role="dialog" aria-modal="true" aria-label={`${activePhoto.angle} progress photo`} onClick={()=>setActivePhoto(null)}><button onClick={()=>setActivePhoto(null)} aria-label="Close photo">×</button><img src={activePhoto.url} alt={`${activePhoto.angle} progress photo enlarged`}/><strong>{activePhoto.angle[0].toUpperCase()+activePhoto.angle.slice(1)} · {formatDate(latest.date)}</strong></div>}</>;
}

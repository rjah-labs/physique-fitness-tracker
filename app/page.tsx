...
import {
  baselineCheckIn, fitnessRepository, measurementFields,
  type CheckIn, type MeasurementKey, type Measurements,
...
export default function Home() {
  ...
  useEffect(() => {
    setCheckIns(fitnessRepository.list());
    if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js").catch(() => undefined);
  }, []);

  function refresh(message: string) {
    setCheckIns(fitnessRepository.list()); setEditing(null); setNotice(message);
    window.setTimeout(() => setNotice(""), 2600);
  }
  function save(entry: CheckIn) { fitnessRepository.save(entry); refresh(editing === "new" ? "Check-in saved on this device" : "Check-in updated"); }
  function remove(entry: CheckIn) { fitnessRepository.remove(entry.id); refresh("Check-in removed"); }

  return <main className="app-shell">
    <header className="topbar"><div className="brand-mark">P</div><div><p className="eyebrow">PHYSIQUE <span className="version">V0.2</span></p><p className="date">{formatDate(latest.date, true)}</p></div><button className="avatar" aria-label="Open profile">RH</button></header>
    ...
    {editing && <CheckInSheet initial={editing === "new" ? latest : editing} isNew={editing === "new"} onClose={() => setEditing(null)} onSave={save} onDelete={remove}/>} 
